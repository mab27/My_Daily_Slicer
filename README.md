...
```sh
python3 trigger_topology.py undeploy         --name Dispo-0524     # POSTs Slicer's undeploy
python3 trigger_topology.py monitor-undeploy --name Dispo-0524     # watch deploy_status walk back to not_deployed
```

Sweep every live topology in one go:
...
`undeploy --all` skips topologies that already have an `undeploy_requested_at`
recorded, so re-running it is safe.
...
`latest` output (includes slicer topology name when available):
```
Dispo-0526: zz-Dispo-0526-evpn_mlag.vex.2485377892354-2254740984
```
...
| `undeploy_requested_at`, `undeploy_started_at`, `undeployed_at` | deprovision chain | Timestamps marking each teardown milestone |
